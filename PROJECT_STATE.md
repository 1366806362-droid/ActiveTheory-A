...
- Human-accepted source `310cffe310eedcaca31d1664aba16900f4f80342`
  contains the complete orbital, particle-star, energy-star and color/depth chain
  over target base `d4f4a2c40b975dcbcbb96e6284b1aeff2629237c`.
- Normal FiveA entry now selects the reviewed combination without candidate query
  parameters. Explicit `fiveAOrbital=0` restores the legacy scene; explicit
  historical A/B candidate parameters remain available. `fiveABackground=0`
  disables only the local FiveA environment.
- The reviewed combination adds
...
- Edge 153 / RTX 5060 Ti / 1600x900 DPR1: separate 60s steady/interaction,
  median/P95/P99 8.3/8.4-8.5/8.5ms; max 8.6/25.0ms, no frames over 50ms.
  Whole-render GPU query median/P95 2.710/3.446ms; Canvas/RAF/Wheel 1/1/1.
- Full integration Node entry: 45 files / 109 runner tests / 689 actual named
  cases, 0 failed/skipped; 8 new cases across the two candidates and promotion.
  Python 6 passed,
  build/diff check passed; existing large-bundle warning retained.
- **HUMAN ACCEPTED / SHARED BASELINE.** Evidence remains untracked in
  `art/fivea-color-depth/`. Data identity remains MOCK/SYNTHETIC: only existing
  stage scale/energy and four transition flowStrength channels are renderer-bound;
  database, real data sources, density and flowSpeed are not completed.
  See `FIVEA_COLOR_DEPTH_REVIEW.md`. HOME meteor remains independently switchable.

## HOME Subtle Meteor Accent — shared baseline decoration — 2026-09-15

- Normal HOME enables the accepted accent without a query parameter. Explicit
  `homeMeteors=0` (or `false`) disables it. No Galaxy, Earth/Hero Lock, labels,
  entries or global postprocessing edit.
...
- **HUMAN ACCEPTED / SHARED BASELINE.** The accent remains independent and
  optional; reduced-motion, inactive scenes and hidden tabs suppress playback
  without catch-up. See `HOME_METEOR_REVIEW.md`.
