...
for(const variant of ['A','B']) {
  test(`${variant}: fixed particle budget, real adapters, idempotence and disposal`,()=>{
    const scene=createFiveAScene({cinematicArt:resolveFiveACinematicArt('?fiveACinematic='+variant),orbitalArt:null});
    tick(scene);
  ...
  test(`${variant}: four alpha-only flows, stable endpoints and missing diagnostics`,()=>{
    const scene=createFiveAScene({cinematicArt:resolveFiveACinematicArt('?fiveACinematic='+variant),orbitalArt:null});tick(scene);
...
test('failed first review publication restores original bindings and fails closed',()=>{
  const scene=createFiveAScene({cinematicArt:resolveFiveACinematicArt('?fiveACinematic=B'),orbitalArt:null});tick(scene);
