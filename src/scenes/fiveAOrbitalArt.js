...
export function resolveFiveAOrbital(search = '') {
  ...
  const requestedOrbital=q.get('fiveAOrbital'),useReviewedDefault=requestedOrbital===null;
  if (!useReviewedDefault&&!['1', 'A', 'B'].includes(requestedOrbital)) return null;
  const colorDepth=resolveFiveAColorDepth(search) ?? (useReviewedDefault
    ? {variant:'A',background:q.get('fiveABackground')!=='0'} : null);
  ...
  return { variant: useReviewedDefault||requestedOrbital==='B' ? 'B' : 'A', skeleton: q.get('orbitalSkeleton') === '1',
