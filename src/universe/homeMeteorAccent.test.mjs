...
import {createMeteorClock,HOME_METEOR_PATHS,meteorPathClear,createHomeMeteorAccent,resolveHomeMeteorAccent} from './homeMeteorAccent.js';
test('reviewed HOME meteor is default with explicit off values',()=>{
 assert.equal(resolveHomeMeteorAccent(''),true);assert.equal(resolveHomeMeteorAccent('?homeMeteors=1'),true);
 assert.equal(resolveHomeMeteorAccent('?homeMeteors=0'),false);assert.equal(resolveHomeMeteorAccent('?homeMeteors=false'),false);
});
