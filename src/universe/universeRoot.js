...
import { createHomeMeteorAccent } from './homeMeteorAccent.js';
...
export function createUniverseRoot() {
  ...
  const galaxyV3 = GALAXY_V3_STATE.enabled
    ? createGalaxyV3Root({
      ...
      config: GALAXY_V3_STATE.finalArtDirection
        ...
        ? GALAXY_V3_FINAL_M3_CONFIG
        : (GALAXY_V3_STATE.heroVersion === 'v6'
        ? GALAXY_V3_V6_CONFIG
  ...
  if(readDebugFlag('homeMeteors',false)){
  ...
  return {
    ...
    gpuGalaxyParticleCount: gpuGalaxy?.particleCount ?? 0,
    businessNebulaPointCount: galaxyPlanets.pointCount,
    earthHorizon,
