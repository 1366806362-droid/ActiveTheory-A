...
export function createFiveATransitionsDemoSnapshot(transitionId, state = 'baseline') {
  const rates = { low: 0.1, baseline: 0.5, high: 0.9, partial: null };
  const ids = ['A1_TO_A2', 'A2_TO_A3', 'A3_TO_A4', 'A4_TO_A5'];
  if (!ids.includes(transitionId) || !Object.hasOwn(rates, state)) throw new Error('Invalid FiveA transition demo');
  const snapshot = structuredClone(CANONICAL_FIVE_A_MOCK);
  snapshot.metadata.snapshotId += `:fivea-transition-${transitionId}-${state}`;
  // A known synthetic cohort — never inferred from point-in-time stage populations.
  const point = (value, field) => ({ value, source: value === null ? null : `MOCK:${transitionId}-cohort:${field}`,
    confidence: value === null ? null : 1, verificationStatus: value === null ? 'MISSING' : 'SYNTHETIC' });
...
export function resolveFiveATransitionsDemo(search, development) {
  const params = new URLSearchParams(search);
  const transitionId = development ? params.get('v2FiveATransition') : null;
  const state = development ? params.get('v2FiveATransitionState') : null;
  if (!transitionId && !state) return null;
  if (!transitionId || !state) throw new Error('FiveA transition and state are both required');
  const frame = params.get('v2FiveATransitionFrame');
  if (frame !== null && !['0', '1', '2'].includes(frame)) throw new Error('Invalid transition capture frame');
  return { transitionId, state, snapshot: createFiveATransitionsDemoSnapshot(transitionId, state),
    capture: frame !== null, sampleTime: frame === null ? null : 12 + Number(frame) * 0.25 };
}
