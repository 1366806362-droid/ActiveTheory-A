...
const base=process.env.EARTH_GATE_BASE||'http://127.0.0.1:5187/';
const out=path.resolve(__dirname,'..',process.env.EARTH_GATE_OUT||'art/earth-hybrid/final');
...
const layers=[['HOME',''],['CLOSEUP','debugEarthV3Closeup=1'],['ISOLATED','earthHybridIsolated=1'],...['depth','cloud','atmosphere'].map(k=>[k.toUpperCase()+'_DEBUG','debugEarthV3Closeup=1&earthHybridDebug='+k])];
const baseline=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../art/earth-hybrid/baselines/baseline-802f/runtime-report.json'),'utf8'));
const selected=process.env.EARTH_CANDIDATE||'B';
const extra='&earthHybrid=1';
const only=process.env.EARTH_GATE_ONLY;
const cases=layers.filter(([n])=>!only||n===only).map(([n,q])=>['EARTH_HYBRID_'+n,'earthOrbital=1&earthV13=1&earthV13Candidate='+selected+'&'+q+extra]);
async function state(p){return p.evaluate(async()=>{
  ...
  return {projection,camera:{position:camera.position.toArray(),quaternion:camera.quaternion.toArray(),fov:camera.fov,aspect:camera.aspect},rotation:root.rotation.toArray(),gpu:gl.getParameter(ext.UNMASKED_RENDERER_WEBGL),fps:samples.length*1000/samples.reduce((n,s)=>n+s.dt,0),
    ...
    loop:(await import('/src/engine/loop.js')).getLoopStatus(),earth:window.__ACTIVE_THEORY_EARTH_V3__,hybrid:window.__ACTIVE_THEORY_EARTH_HYBRID__,
...
(async()=>{
  ...
  try{
    ...
    for(const [name,extra] of cases){
      ...
      await p.waitForFunction(()=>window.__ACTIVE_THEORY_EARTH_HYBRID__?.ready);
      await p.mouse.move(800,450);await p.waitForTimeout(3100);
      if(name==='EARTH_HYBRID_ISOLATED')await p.evaluate(async()=>{
        const scene=(await import('/src/engine/scenes.js')).getActiveScene(),root=scene.getObjectByName('EarthRoot');
        const keep=new Set();root.traverse(o=>keep.add(o));let p=root;while(p){keep.add(p);p=p.parent;}
        const before=scene.onBeforeRender;
        scene.onBeforeRender=function(...args){before.apply(this,args);scene.traverse(o=>{if(!keep.has(o)&&(o.isMesh||o.isPoints||o.isLine||o.isSprite))o.visible=false;});};
        for(const el of document.body.querySelectorAll('*'))if(el.tagName!=='CANVAS'&&!el.querySelector('canvas'))el.style.visibility='hidden';
        document.querySelector('canvas').style.visibility='visible';
        const style=document.createElement('style');style.textContent='body *{visibility:hidden!important}body canvas{visibility:visible!important}';document.head.append(style);
      });
      ...
      if(name==='EARTH_HYBRID_HOME'||name==='EARTH_HYBRID_CLOSEUP'){
      ...
      if(name==='EARTH_HYBRID_HOME')for(const [label,x,y] of [['LEFT',720,440],['CENTER',800,450],['RIGHT',880,460]]){
        await p.mouse.move(x,y);await p.waitForTimeout(650);report.parallax.push({label,...await state(p)});await p.screenshot({path:path.join(out,`EARTH_HYBRID_${label}.png`)});
      }
    }
    if(!only)for(const name of ['HOME','CLOSEUP']){
      const x=report.cases.find(c=>c.name==='EARTH_HYBRID_'+name),y=baseline.cases.find(c=>c.name==='EARTH_V12_'+name);
  ...
  }finally{fs.writeFileSync(path.join(out,only?'capture-'+only+'-report.json':'runtime-report.json'),JSON.stringify(report,null,2));await b.close()}
