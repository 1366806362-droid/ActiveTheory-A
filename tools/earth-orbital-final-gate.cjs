...
const base=process.env.EARTH_GATE_BASE||'http://127.0.0.1:5185/';
const out=path.resolve(__dirname,'..',process.env.EARTH_GATE_OUT||(process.argv.includes('--candidates')?'art/earth-v12-final/candidates':'art/earth-v12-final/final'));
...
const baseline=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../art/earth-v12-final/baseline-802f/runtime-report.json'),'utf8'));
const selected=process.env.EARTH_CANDIDATE||'B';
const cases=process.argv.includes('--candidates')
  ? ['A','B','C'].flatMap(c=>layers.slice(0,2).map(([n,q])=>['CANDIDATE_'+c+'_'+n,'earthOrbital=1&earthOrbitalCandidate='+c+'&'+q]))
  : [...layers.map(([n,q])=>['EARTH_V12_FINAL_'+n,'earthOrbital=1&earthOrbitalCandidate='+selected+'&'+q])];
...
(async()=>{
  ...
  try{
    ...
    for(const [name,extra] of cases){
      ...
      if(name==='EARTH_V12_FINAL_HOME')for(const [label,x,y] of [['LEFT',720,440],['CENTER',800,450],['RIGHT',880,460]]){
        await p.mouse.move(x,y);await p.waitForTimeout(650);report.parallax.push({label,...await state(p)});await p.screenshot({path:path.join(out,`EARTH_V12_FINAL_${label}.png`)});
      }
    }
    if(!process.argv.includes('--candidates'))for(const name of ['HOME','CLOSEUP']){
      const x=report.cases.find(c=>c.name==='EARTH_V12_FINAL_'+name),y=baseline.cases.find(c=>c.name==='EARTH_V12_'+name);
