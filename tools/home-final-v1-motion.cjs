...
const out=path.resolve(__dirname,'..','art/home-final-v1/final');
(async()=>{
  const url=JSON.parse(fs.readFileSync(path.join(out,'runtime-report.json'),'utf8')).url;
  ...
  const seconds=(end-begin)/1000,destination=path.join(out,'HOME_FINAL_V1_PARALLAX.mp4');
