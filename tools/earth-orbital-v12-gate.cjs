...
const out=path.resolve(__dirname,'..',process.env.EARTH_GATE_OUT||'art/earth-v12');
...
const cases=process.argv.includes('--candidates')
  ? ['A','B','C'].flatMap(c=>layers.map(([n,q])=>['CANDIDATE_'+c+'_'+n,'earthOrbital=1&earthOrbitalCandidate='+c+'&'+q]))
  : [...layers.map(([n,q])=>['EARTH_V12_'+n,'earthOrbital=1&earthOrbitalCandidate='+selected+'&'+q]),
    ['FROZEN_HOME',''],['FROZEN_CLOSEUP','debugEarthV3Closeup=1']];
...
(async()=>{
  ...
  try{
    ...
    const capturePoses={};report.parallax=[];
    for(const [name,extra] of cases){
      ...
      if(name==='EARTH_V12_HOME')for(const [label,x,y] of [['LEFT',720,440],['CENTER',800,450],['RIGHT',880,460]]){
        await p.mouse.move(x,y);await p.waitForTimeout(650);report.parallax.push({label,...await state(p)});await p.screenshot({path:path.join(out,`EARTH_V12_${label}.png`)});
      }
    }
    if(!process.argv.includes('--candidates'))for(const name of ['HOME','CLOSEUP']){
      const x=report.cases.find(c=>c.name==='EARTH_V12_'+name),y=report.cases.find(c=>c.name==='FROZEN_'+name);
