...
const out=path.resolve(__dirname,'..','art/earth-v12-final/final');
(async()=>{
  const url=JSON.parse(fs.readFileSync(path.join(out,'runtime-report.json'),'utf8')).cases.find(c=>c.name==='EARTH_V12_FINAL_CLOSEUP').url.replace('&earthFreeze=1','').replace('&earthAudit=1','');
  ...
  const seconds=(end-begin)/1000,destination=path.join(out,'EARTH_V12_FINAL_ORBITAL.mp4');
