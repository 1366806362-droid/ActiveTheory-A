...
const out=path.resolve('art/brandmind-v11');fs.mkdirSync(out,{recursive:true});
const base=process.env.BRANDMIND_URL||'http://127.0.0.1:5197/';
const mode=process.argv[2]||'baseline';
const report={mode,errors:[]};
const candidate='brandMindCognitiveV11=B&brandMindCognitiveReview=1';
...
(async()=>{const b=await chromium.launch({channel:'msedge',headless:false});try{const c=await b.newContext({viewport:{width:1600,height:900},deviceScaleFactor:1}),p=await c.newPage();p.on('pageerror',e=>report.errors.push(e.message));p.on('console',m=>{if(m.type()==='error')report.errors.push(m.text());});
 ...
 if(mode==='candidates'){report.candidates={};for(const variant of ['A','B']){await go(p,`scene=brandmind&brandMindCognitiveV11=${variant}&brandMindCognitiveReview=1`);await p.evaluate(()=>{const r=window.__BRANDMIND_COGNITIVE_REVIEW__;r.sample(12);});await shot(p,'CANDIDATE_'+variant);report.candidates[variant]=await inspect(p);await clickCore(p);await shot(p,'PANEL_'+variant);}}
 if(mode==='final'){
  await go(p,'scene=brandmind&'+candidate);await p.evaluate(()=>window.__BRANDMIND_COGNITIVE_REVIEW__.sample(12));await p.waitForTimeout(100);await shot(p,'BRANDMIND_V11_OVERVIEW');report.after=await inspect(p);
  await p.screenshot({path:path.join(out,'BRANDMIND_V11_CORE_DETAIL.png'),clip:{x:590,y:290,width:500,height:370}});
  await p.screenshot({path:path.join(out,'BRANDMIND_V11_ASSOCIATIONS.png'),clip:{x:250,y:155,width:1080,height:660}});
  await p.screenshot({path:path.join(out,'BRANDMIND_V11_FLOW_DETAIL.png'),clip:{x:350,y:260,width:590,height:310}});
  ...
  for(const [width,height]of [[1366,768],[1600,900],[1920,1080]]){await p.setViewportSize({width,height});await clickCore(p);await shot(p,'PANEL_'+width);if(width===1600)await shot(p,'BRANDMIND_V11_PANEL_OPEN');await p.keyboard.press('Escape');await p.waitForTimeout(1400);}
  await p.setViewportSize({width:1600,height:900});await go(p,'scene=brandmind&'+candidate+'&showBloom=0');await p.evaluate(()=>window.__BRANDMIND_COGNITIVE_REVIEW__.sample(12));await p.waitForTimeout(100);await shot(p,'BLOOM_OFF');
  await go(p,'scene=brandmind&'+candidate);await p.evaluate(()=>window.__BRANDMIND_COGNITIVE_REVIEW__.sample(12));await p.waitForTimeout(100);await shot(p,'BLOOM_ON');
  await go(p,'brandMindCognitiveV11=B&v2FiveAState=balanced&v2FiveACapture=1&homeMeteors=0');await shot(p,'HOME_CANDIDATE_FIXED');
 ...
 if(mode==='perf'){
  ...
  for(const [name,query]of [['BEFORE','scene=brandmind'],['STEADY','scene=brandmind&brandMindCognitiveV11=B'],['INTERACTION','scene=brandmind&brandMindCognitiveV11=B']].filter(([name])=>!process.env.BRANDMIND_PERF_CASE||process.env.BRANDMIND_PERF_CASE===name)){
 ...
 if(mode==='environment'){
  ...
  await go(p,'scene=brandmind&brandMindCognitiveV11=0&v2FiveAState=balanced&v2FiveACapture=1');await shot(p,'FALLBACK_FINAL_FIXED');
 }
 if(mode==='video'||mode==='baseline-motion'){
  ...
  if(mode==='video'){await p.waitForTimeout(1000);mark('HOME click Brand Mind');await enter(p);mark('Cognitive Core and association fibers');await p.waitForTimeout(18000);mark('Core click');await clickCore(p);await p.waitForTimeout(5500);mark('ESC close');await p.keyboard.press('Escape');await p.waitForTimeout(2500);mark('Return HOME');await back(p);}else await p.waitForTimeout(10000);
  ...
  const ff=spawnSync('ffmpeg',['-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',manifest,'-fps_mode','vfr','-c:v','libx264','-threads','2','-crf','16','-pix_fmt','yuv420p','-movflags','+faststart',path.join(out,mode==='video'?'BRANDMIND_V11_DEMO.mp4':'BEFORE_MOTION.mp4')],{encoding:'utf8'});assert.equal(ff.status,0,ff.stderr);
 }
