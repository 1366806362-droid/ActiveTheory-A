...
$configPath = Join-Path $PSScriptRoot 'galaxy_hero_v1_config.json'
$builderPath = Join-Path $PSScriptRoot 'build_galaxy_hero_v1.py'
$rendererPath = Join-Path $PSScriptRoot 'render_galaxy_hero_v1.py'
$outputDir = Join-Path $repoRoot 'art\galaxy-v3\hero-v1'
$scenePath = Join-Path $outputDir 'galaxy-hero-v1.blend'
$buildReport = Join-Path $outputDir 'galaxy-hero-v1-build.json'
$renderLog = Join-Path $outputDir 'galaxy-hero-v1-render-log.json'
...
if (-not $homeGpuMatched) { throw 'RTX 5060 Ti is required for this render.' }
New-Item -ItemType Directory -Force -Path $outputDir | Out-Null

& $BlenderPath '--background' '--factory-startup' '--python-exit-code' '1' '--python' $builderPath '--' `
  '--config' $configPath '--output' $scenePath '--report' $buildReport
if ($LASTEXITCODE -ne 0) { throw "Galaxy Hero scene build failed with exit code $LASTEXITCODE" }

$plan = [ordered]@{
  schemaVersion = '1.0.0'
  mode = if ($DryRun) { 'DRY_RUN' } else { 'RENDER_LOOKDEV' }
  ...
  samples = 64
  outputs = @(
    (Join-Path $outputDir 'GALAXY_HERO_V1_RGBA.png'),
    (Join-Path $outputDir 'GALAXY_HERO_V1_PREVIEW.png')
  )
}
$plan | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $outputDir 'galaxy-hero-v1-plan.json') -Encoding utf8
if ($DryRun) {
...
if ($null -ne $exitCode -and [int]$exitCode -ne 0) { throw "Galaxy Hero render failed with exit code $exitCode" }
